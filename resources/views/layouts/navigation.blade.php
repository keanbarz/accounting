    ...
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <!-- Navigation Links -->
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    @if (Auth::user()->role == 'Accounting Receiving Clerk')
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" style="color:green">
                    {{ __('Incoming Vouchers') }}
                    </x-nav-link>
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" style="color:green">
                    {{ __('Outgoing Vouchers') }}
                    </x-nav-link>
                    @else
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" style="color:green">
                    {{ __('Disbursement Vouchers') }}
                    @endif
                    </x-nav-link>
                </div>
            </div>
